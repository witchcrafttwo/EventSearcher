...
// .wrangler/tmp/bundle-3nOqab/strip-cf-connecting-ip-header.js
...
// src/event-ai.ts
var CATEGORIES = ["\u796D\u308A\u30FB\u4F1D\u7D71", "\u97F3\u697D\u30FB\u30E9\u30A4\u30D6", "\u30B9\u30DD\u30FC\u30C4", "\u81EA\u7136\u30FB\u30A2\u30A6\u30C8\u30C9\u30A2", "\u30A2\u30FC\u30C8\u30FB\u5C55\u793A", "\u30B0\u30EB\u30E1\u30FB\u30DE\u30EB\u30B7\u30A7", "\u30EF\u30FC\u30AF\u30B7\u30E7\u30C3\u30D7", "\u6587\u5316\u30FB\u8B1B\u6F14", "\u30C7\u30D1\u30FC\u30C8\u30FB\u30E2\u30FC\u30EB", "\u305D\u306E\u4ED6"];
...
function mergeParsedEvent(parsed, fallback) {
  return {
    ...
    eventDate: normalizeDate2(parsed.eventDate) ?? fallback.eventDate,
    eventEndDate: normalizeDate2(parsed.eventEndDate) ?? fallback.eventEndDate,
...
function findPaginationUrls(html, baseUrl) {
  let base;
  try {
...
__name(findPaginationUrls, "findPaginationUrls");
function escapeRegExp(value) {
  return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
__name(escapeRegExp, "escapeRegExp");
function isYearArchivePath(path) {
  return /\/(19|20)\d{2}(\/\d{1,2})?\/?$/.test(path);
}
__name(isYearArchivePath, "isYearArchivePath");
...
function parseRss(source, xml) {
  ...
  return items.slice(0, 30).map((item) => {
    ...
    return {
      ...
      publishedAt: normalizeDate3(publishedAt)
    };
...
async function updateSource(env2, id, patch) {
  if (!id)
    throw new Error("id is required");
  const ddb = new DynamoClient(env2);
  let existing = await ddb.getItem(env2.SOURCES_TABLE, { id });
  if (!existing) {
    const all = await loadAllSources(env2);
  ...
  if (patch.forceCategory !== void 0) {
    const value = patch.forceCategory.trim();
    if (value)
      updated.forceCategory = value;
    else
      delete updated.forceCategory;
  }
...
async function loadAllSources(env2) {
  const fromFile = await normalizeFileSources(defaultSources);
...
async function runIngest(env2, options = {}) {
  ...
  const forceCategoryById = /* @__PURE__ */ new Map();
  if (!isDiscoveryEnabled(env2)) {
    for (const s of await loadAllSources(env2)) {
      if (s.forceCategory)
        forceCategoryById.set(s.id, s.forceCategory);
    }
  }
  const newEvents = [];
...
async function clearEvents(env2) {
  ...
  for (let i = 0; i < events.length; i += BATCH) {
    ...
    await Promise.all(slice.map((event) => ddb.deleteItem(env2.EVENTS_TABLE, { eventId: event.eventId })));
  }
...
async function previewIngest(env2, limit) {
  const allSources = isDiscoveryEnabled(env2) ? [] : await loadAllSources(env2);
  const forceCategoryById = /* @__PURE__ */ new Map();
  for (const s of allSources) {
    if (s.forceCategory)
      forceCategoryById.set(s.id, s.forceCategory);
  }
  const candidates = isDiscoveryEnabled(env2) ? await discoverCandidates(env2) : await fetchCandidates(allSources);
...
app.get("/cron/ingest", async (c) => {
  ...
  c.executionCtx.waitUntil(
    runScheduledIngest(c.env).catch((error3) => console.error("cron ingest failed", error3))
  );
...
app.get("/admin/stats", async (c) => {
  ...
  const hostToId = /* @__PURE__ */ new Map();
...
app.get("/admin/ai-debug", async (c) => {
  ...
  if (url) {
    ...
    const candidate = { sourceId: "debug", sourceName: "debug", sourceUrl: url, title: "debug", url, area: "", snippet: buildAiText(text), publishedAt: (/* @__PURE__ */ new Date()).toISOString() };
...
async function buildDisabledMatcher(env2) {
  return buildDisabledMatcherFrom(await loadAllSources(env2));
}
__name(buildDisabledMatcher, "buildDisabledMatcher");
function buildDisabledMatcherFrom(sources) {
...
__name(buildDisabledMatcherFrom, "buildDisabledMatcherFrom");
function buildForcedCategory(sources) {
  const forced = sources.filter((s) => s.forceCategory);
  const byId = new Map(forced.map((s) => [s.id, s.forceCategory]));
  const byHost = new Map(forced.map((s) => [hostOf(s.url), s.forceCategory]).filter(([h]) => h));
  return (event) => byId.get(event.sourceId) ?? byHost.get(hostOf(event.url));
}
__name(buildForcedCategory, "buildForcedCategory");
function hostOf(url) {
  try {
...
// .wrangler/tmp/bundle-3nOqab/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3nOqab/middleware-loader.entry.ts
