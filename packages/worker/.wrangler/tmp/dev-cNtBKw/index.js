...
// .wrangler/tmp/bundle-jIPWpC/strip-cf-connecting-ip-header.js
...
async function converse(env2, prompt) {
  ...
  const url = `https://bedrock-runtime.${env2.AWS_REGION}.amazonaws.com/model/${encodeURIComponent(modelId)}/converse`;
...
async function enrichCandidate(env2, candidate) {
  ...
  try {
    const text = await converse(env2, buildPrompt(candidate));
...
var EVENT_WORDS = ["\u30A4\u30D9\u30F3\u30C8", "\u8B1B\u5EA7", "\u4F53\u9A13", "\u30EF\u30FC\u30AF\u30B7\u30E7\u30C3\u30D7", "\u5C55\u793A", "\u307E\u3064\u308A", "\u89AA\u5B50", "\u5B50\u3069\u3082", "\u3053\u3069\u3082"];
...
function parseHtml(source, html) {
  ...
  while ((match2 = anchorPattern.exec(html)) !== null) {
    const href = match2[1];
    const title2 = normalizeWhitespace(stripTags(match2[2]));
    if (!title2 || !href || title2.length < 5 || title2.length > 120)
      continue;
    if (!EVENT_WORDS.some((word) => title2.includes(word)))
      continue;
...
// src/ingest.ts
async function runIngest(env2) {
  const ddb = new DynamoClient(env2);
  const candidates = isDiscoveryEnabled(env2) ? await discoverCandidates(env2) : await fetchCandidates(parseSources(env2.EVENT_SOURCES_JSON ?? "[]"));
  ...
  for (const candidate of candidates) {
    ...
    if (exists)
      continue;
    const enriched = await enrichCandidate(env2, candidate);
    const event = {
      ...enriched,
      eventId,
      eventType: "event",
    ...
    await ddb.putItem(env2.EVENTS_TABLE, event, "attribute_not_exists(eventId)");
...
// .wrangler/tmp/bundle-jIPWpC/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-jIPWpC/middleware-loader.entry.ts
