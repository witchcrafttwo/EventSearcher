...
現在のWeb Push通知は収集処理の中で即時送信されるため、収集を実行した時刻がそのまま通知時刻になり、深夜に通知が届く可能性がある。本仕様では収集処理と通知配信処理を分離し、日本時間19:00を通知配信時刻、日本時間22:00〜08:00を通知禁止時間帯として送信を制御する。ホスティング構成は変更せず、現行のVercel、ローカル収集CLI、自宅サーバー用エントリのいずれでも同じ挙動となるよう共通処理として実装する。
...
- **通知配信時刻**: 未通知イベントをまとめてPush送信する時刻。日本時間19:00とする。
- **通知禁止時間帯**: Web Pushを送信しない時間帯。日本時間22:00以降08:00未満とする。
- **滞留イベント**: 未通知のまま既定日数（7日）を超えたイベント。送信せず通知済みにして解消する。
...
### Requirement 2: 通知配信時刻
**User Story:** 利用者として、毎日決まった時間に新着イベントを受け取りたい。

#### Acceptance Criteria
1. THE SYSTEM SHALL 通知配信時刻を日本時間19:00とする。
2. THE SYSTEM SHALL 通知配信処理を1日1回起動する。
3. THE SYSTEM SHALL 通知配信時刻を設定値として変更可能にする。
4. WHEN 通知配信処理が起動したとき THEN システム SHALL その時点の未通知イベントをまとめて送信対象とする。
5. THE SYSTEM SHALL 収集処理をどの時刻に実行しても通知配信時刻が変わらないようにする。

### Requirement 3: 通知禁止時間帯の抑止
...
### Requirement 5: 通知対象イベントの鮮度
**User Story:** 利用者として、すでに終了したイベントの通知を受け取りたくない。

#### Acceptance Criteria
1. WHEN 通知配信処理が送信対象を選ぶとき THEN システム SHALL 開催終了済みのイベントを除外する。
2. IF 未通知イベントが保存から7日を超えて滞留した THEN システム SHALL 当該イベントを送信せず通知済みとして記録し、保留を解消する。
3. THE SYSTEM SHALL 滞留と判断する日数の既定値を7日とし、設定値として変更可能にする。

### Requirement 6: 購読の健全性維持
